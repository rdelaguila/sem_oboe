...
class KGEModelTrainer:
    ...
    def train_kge_model_single_dim(self, training_tf: TriplesFactory, testing_tf: TriplesFactory,
        ...
        # Use CPU instead of MPS for stability (MPS can hang on some operations)
        device = "mps"  # Force CPU for stable training
        print(f"Using device: {device}")

        # Enhanced stopper kwargs - use validation instead of testing for early stopping
        stopper_kwargs = {
            'frequency': 3,  # Check every 3 epochs (faster)
            'patience': 3,  # Stop after 3 evaluations without improvement (faster)
            'relative_delta': 0.002,  # 0.2% minimum improvement
        ...
        result = pipeline(
            ...
            training_kwargs=dict(
                ...
                use_tqdm_batch=False,  # Disable batch progress bar
            ),
            stopper='early',
...
def run_complete_pipeline(kg_path: str, emb_dir: str,
                          ...
                          n_iter: int = 20,
                          cv: int = 3,
                          kge_loss_threshold: float = 0.001) -> Tuple[XGBClassifier, Dict[str, Any]]:
    """
...
def example_complete_pipeline():
    """Example of running both phases sequentially with loss threshold"""
    best_model, metrics = run_complete_pipeline(
        ...
        n_iter=25,
        cv=3,
        kge_loss_threshold=0.001  # KGE loss threshold
    )
