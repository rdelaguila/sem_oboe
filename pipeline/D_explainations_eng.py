...
def main():
    ...
    TERMINOS_A_INCLUIR = set([term.lower() for term in config['vocabulary']])  # Convert to lowercase
    ...
    if TOPIC_ID in vocabulary_dict:
        ...
        # so they can be found in triplets
    ...
    # Show some examples
    if dictdbp:
        print(f"   DBPedia examples: {list(dictdbp.keys())[:5]}")
    else:
        print("   ⚠️ PROBLEM: DBPedia dictionary is empty!")
        print("   Checking vocabulary_dict content...")
        print(f"   - Topic ID: {TOPIC_ID}")
        print(f"   - Vocabulary dict keys: {list(vocabulary_dict.keys())}")
        if TOPIC_ID in vocabulary_dict:
            print(f"   - DBPedia entities for topic {TOPIC_ID}: {vocabulary_dict[TOPIC_ID]['dbpedia'][:10]}")
            print(f"   - NER entities for topic {TOPIC_ID}: {vocabulary_dict[TOPIC_ID]['ner'][:10]}")

        # As fallback, create dictdbp from selected vocabulary
        print("   Creating dictionary from selected vocabulary as fallback...")
        for term in config['vocabulary']:
            term_lower = term.lower()
            dictdbp[term_lower] = [{'URI': f'http://dbpedia.org/resource/{term}', 'tipos': []}]
        print(f"   ✅ Fallback dictionary created with {len(dictdbp)} terms")

    if dictner:
        print(f"   NER examples: {list(dictner.keys())[:5]}")
    else:
        print("   ⚠️ NER dictionary is empty (normal if no NER entities)")

    # Additional vocabulary debugging
    print(f"\nDebugging selected vocabulary:")
    print(f"   - Vocabulary type: {type(config['vocabulary'])}")
    print(f"   - First 10 terms: {config['vocabulary'][:10]}")
    print(f"   - Terms in lowercase: {[term.lower() for term in config['vocabulary'][:5]]}")
    ...
    try:
        ...
        print(
            f"❌ Error: SpaCy model '{SPACY_MODEL}' not found. Install it with: python -m spacy download {SPACY_MODEL}")
        return
    ...
    try:
        ...
        # Show triplet sample for debugging
    ...
    print(f"DBPedia terms for filtering: {len(palabrasdbpedia)}")
    print(f"NER terms for filtering: {len(dictner)}")
    print(f"Vocabulary to include: {len(TERMINOS_A_INCLUIR)}")

    # Debug: show intersections
    if palabrasdbpedia:
        interseccion = TERMINOS_A_INCLUIR.intersection(palabrasdbpedia)
        print(f"Vocabulary-DBPedia intersection: {len(interseccion)} terms")
        if interseccion:
            print(f"   Intersection examples: {list(interseccion)[:5]}")
    else:
        print("No DBPedia words to intersect!")
    ...
    for i, row in df_tr.iterrows():
        if processed_count % 5000 == 0:  # Reduce logging frequency
            print(
                f"  Processing: {processed_count}/{len(df_tr)} (terms found: {terms_found_count}, matches: {matches_found})")
        processed_count += 1
        ...
        if has_match:
            ...
            # Debug: show first matches
            if matches_found <= 5:
                print(f"     Match {matches_found}: subject={sujeto}, object={objeto}")
                print(f"       Subject intersection: {sujeto.intersection(TERMINOS_A_INCLUIR)}")
                if VISITAR_OBJETO:
                    print(f"       Object intersection: {objeto.intersection(TERMINOS_A_INCLUIR)}")
            ...
            # Debug: show found terms
            if matches_found <= 5 and encontradas:
                print(f"       Found in DBPedia: {encontradas}")
            ...
            for termino in encontradas:
                ...
                # Debug: show first terms found
                if terms_found_count <= 5:
                    print(f"       ✅ Term processed: {termino}")

    print(f"Final processing statistics:")
    print(f"   - Triplets processed: {processed_count}")
    print(f"   - Matches with vocabulary: {matches_found}")
    print(f"   - Final extracted terms: {terms_found_count}")

    print(f"✅ Triplets processed: {len(listado_tripletas)} relevant terms found")

    if len(listado_tripletas) == 0:
        print("❌ No relevant terms found. Check the selected vocabulary.")
        print("Debugging information:")
        print(f"  - Vocabulary to include: {list(TERMINOS_A_INCLUIR)[:10]}...")
        print(f"  - Available DBPedia words: {list(palabrasdbpedia)[:10]}...")
        print(f"  - Total triplets processed: {processed_count}")
        return
    ...
    for _, row in df.iterrows():
        ...
        try:
            ...
            print(f"  ⚠️ Error processing term '{termino}': {e}")
            continue
    ...
    plt.title(f"Hierarchical Dendrogram (Ward) - {final_clusters} clusters selected")
    plt.xlabel('Terms')
    plt.ylabel('Distance')
    ...
    print(f"Dendrogram with cut generated in {OUTPUT_DIR}")
    ...
    # 4. GENERATION WITH ADVANCED PROMPTING STRATEGIES (based on QualIT + XAI)
    print("\nInitializing AI models for generation and evaluation...")
    ...
    def generate_explanation_with_cot(cluster_id, terms, topic_id, silhouette_score):
        ...
        Based on QualIT: key phrase extraction -> verification -> explanation
        """
        if gen is None:
            return "Explanation not available - model not loaded", "", False

        # Step 1: Key phrase extraction (inspired by QualIT)
...
KEY PHRASES:"""
        ...
        # Step 2: Relevance verification (anti-hallucination, QualIT style)
...
VERIFICATION RESULT (True/False): """
        ...
        # Step 3: Final explanation generation with structured reasoning
...
JSON Response:"""
    ...
    for cid, terms_c in clusters.items():
        try:
            ...
            if parsed is None:
                # Fallback with process information
                parsed = {
...
JSON Evaluation:"""
    ...
    try:
        ...
        print("⚠️ Using simplified evaluation")
        evalp = None
    ...
    for cid, exp in explanations.items():
        ...
        # Use main generative for more consistent evaluation
        ...
        try:
            ...
            print(f"  ❌ Error evaluating cluster {cid}: {e}")
            parsed = {
                'coherence': 3,
                'relevance': 3,
                'coverage': 3,
    ...
    with open(os.path.join(OUTPUT_DIR, 'summary.txt'), 'w', encoding='utf-8') as f:
        ...
        f.write(f"Selected vocabulary: {len(config['vocabulary'])} terms\n")
        f.write(f"Number of identified clusters: {len(clusters)}\n")
        f.write(f"Automatic optimal number: {optimal_clusters}\n")
        f.write(f"Number selected by user: {final_clusters}\n")
        ...
        f.write(f"  • Total terms used: {stats['total_terms']}\n")
        f.write(f"  • Available LDA terms: {stats['lda_available']}\n")
        f.write(f"  • Available NER entities: {stats['ner_available']}\n")
        f.write(f"  • Available DBPedia entities: {stats['dbpedia_available']}\n\n")
        ...
        f.write(f"  • clusters.json - Cluster information\n")
        f.write(f"  • explanations.json - Generated explanations\n")
        f.write(f"  • evaluations.json - XAI evaluations\n")
        f.write(f"  • detailed_analysis.json - Detailed analysis\n")
        f.write(f"  • silhouette_evolution.png - Optimization graph\n")
        f.write(f"  • dendrogram_with_cut.png - Dendrogram\n")
        f.write(f"  • config_topic_{TOPIC_ID}.json - Configuration\n")
    ...
    print("=" * 50)
    print(f"Output directory: {OUTPUT_DIR}")
    print(f"{len(clusters)} clusters generated with silhouette {global_sil:.3f}")
    print(f"Topic {TOPIC_ID} from repository '{config['repo_name']}'")
    if coherences:
        print(f"Average quality: Coherence {np.mean(coherences):.1f}/5")
    print("\nGenerated files:")
    print("  • summary.txt - Complete executive summary")
    print("  • clusters.json - Detailed cluster information")
    print("  • explanations.json - Chain-of-Thought explanations")
    print("  • evaluations.json - XAI criteria evaluations")
    print("  • PNG graphics - Clustering visualizations")
