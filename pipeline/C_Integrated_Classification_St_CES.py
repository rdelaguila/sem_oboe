...
class IntegratedClassificationPipeline:
    ...
    def run_classification_with_statistics(self, n_iter: int = 20, cv: int = 3) -> Tuple[Any, Dict]:
        ...
        self.kge_optimizer.entity_to_id = training_tf.entity_to_id
...
def run_one_model_all_repos(model: str, base_path: str = '../data',
    ...
    for repo, dataset in REPOSITORIES.items():
        ...
        emb_dir = f"{base_path}/triples_emb/{repo}_ces_{model}"
        output_dir = f"{base_path}/model_output/{repo}_ces_{model}"
...
if __name__ == "__main__":
    ...
    if choice == '1':
        ...
        emb_dir = f"../data/triples_emb/{repo}_ces_{model}"
        output_dir = f"../data/model_output/{repo}_ces_{model}"
