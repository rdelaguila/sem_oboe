...
shc.dendrogram(shc.linkage(M, method='ward'), labels=terms)
plt.title("Dendrograma jerárquico de términos")
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'dendrogram.png'))
plt.close()

# Clustering final
labels = AgglomerativeClustering(n_clusters=optimal_clusters).fit_predict(M)
