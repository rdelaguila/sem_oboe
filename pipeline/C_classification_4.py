...
class KGEModelTrainer:
    """
    Phase 1: Knowledge Graph Embeddings (KGE) model training with loss-based early stopping
    """
    ...
    def __init__(self, kg_path: str, emb_dir: str,
                 ...
                 embedding_dims: List[int] = None, loss_threshold: float = 0.001):
        ...
        self.loss_threshold = loss_threshold  # NEW: Loss threshold for early stopping
    ...
    def train_kge_model_single_dim(self, training_tf: TriplesFactory, testing_tf: TriplesFactory,
        ...
        """Train the KGE model with a specific embedding dimension and loss-based early stopping"""
        print(f"\nTraining {self.AVAILABLE_MODELS[self.model_type]} model with embedding_dim={embedding_dim}...")
        print(f"Early stopping: patience=5, will monitor if loss < {self.loss_threshold}")
        ...
        # Enhanced stopper kwargs - use validation instead of testing for early stopping
        stopper_kwargs = {
        ...
        # Split training into train/validation for proper early stopping
        training_sub_tf, validation_tf = training_tf.split([0.9, 0.1], random_state=0)
        ...
        result = pipeline(
            ...
            validation=validation_tf,  # Add validation set for early stopping
            testing=testing_tf,
        ...
        # Check final loss from training
        final_loss = None
        if hasattr(result, 'losses') and result.losses:
            final_loss = result.losses[-1]
            print(f"\nFinal training loss: {final_loss:.6f}")
            if final_loss < self.loss_threshold:
                print(f"✓ Loss threshold achieved! ({final_loss:.6f} < {self.loss_threshold})")
        ...
        if final_loss is not None:
            kge_metrics['final_loss'] = float(final_loss)
            kge_metrics['loss_threshold_reached'] = final_loss < self.loss_threshold
        ...
        if 'final_loss' in kge_metrics:
            print(f"Final Loss: {kge_metrics['final_loss']:.6f}")
    ...
    def train_kge_model(self, training_tf: TriplesFactory, testing_tf: TriplesFactory) -> Dict[str, Any]:
        ...
        print(f"Loss threshold for early stopping: {self.loss_threshold}")
        print("=" * 60)
        ...
        if 'final_loss' in best_metrics:
            print(f"Best final loss: {best_metrics['final_loss']:.6f}")
        print("=" * 60)
    ...
    def save_kge_model_and_mappings(self, kge_metrics: Dict[str, Any]) -> str:
        ...
        basic_info = {
            ...
            'loss_threshold': self.loss_threshold,
            'final_loss': float(kge_metrics.get('final_loss', 0.0)) if 'final_loss' in kge_metrics else None
        }
        ...
        with open(self.kge_metrics_file, 'w') as f:
            ...
            f.write(f"Loss Threshold: {kge_metrics['loss_threshold']}\n")
            if 'final_loss' in kge_metrics:
                f.write(f"Final Loss: {kge_metrics['final_loss']:.6f}\n")
                f.write(f"Threshold Reached: {kge_metrics.get('loss_threshold_reached', False)}\n")
            f.write("\n")
...
class TopicClassifier:
    """
    Phase 2: Topic classification using KGE embeddings with XGBoost early stopping
    """

    def __init__(self, kge_csv_path: str, model_path: str, mappings_path: str,
                 output_dir: str = "data/model_output/amazon",
                 xgb_loss_threshold: float = 0.001):
        self.kge_csv_path = kge_csv_path
        self.model_path = model_path
        self.mappings_path = mappings_path
        self.output_dir = output_dir
        self.xgb_loss_threshold = xgb_loss_threshold  # NEW: XGBoost loss threshold
    ...
    def save_classification_results(self, metrics: Dict[str, Any],
        ...
        results = {
            ...
            'xgb_loss_threshold': self.xgb_loss_threshold,
        ...
        with open(self.metrics_file, 'w') as f:
            ...
            f.write(f"XGBoost Loss Threshold: {self.xgb_loss_threshold}\n")
    ...
    def run_classification_with_grid_search(self, X: np.ndarray, y: np.ndarray,
        ...
        """Run classification with hyperparameter tuning and early stopping based on loss"""
        print("\nPreparing classification with hyperparameter tuning...")
        print(f"XGBoost will use early stopping with loss threshold: {self.xgb_loss_threshold}")
        ...
        param_grid = {
            'n_estimators': [100, 200, 300, 500],  # Increased max estimators
            'max_depth': [3, 5, 7, 10],
        ...
        base_model = XGBClassifier(
            ...
            callbacks=None  # Will be set during fit
        )
        ...
        # Check if loss threshold was reached
        if hasattr(best_model, 'best_score'):
            best_val_loss = -best_model.best_score
            print(f"\nBest validation loss: {best_val_loss:.6f}")
            if best_val_loss < self.xgb_loss_threshold:
                print(f"✓ Loss threshold achieved! ({best_val_loss:.6f} < {self.xgb_loss_threshold})")
    ...
    def run_topic_classification(self, use_random_search: bool = True,
        ...
        print(f"XGBoost Loss Threshold: {self.xgb_loss_threshold}")
        print("=" * 60)
...
def run_kge_training_phase(kg_path: str, emb_dir: str,
                           ...
                           loss_threshold: float = 0.001) -> str:
    """
    Execute Phase 1: KGE Model Training with loss-based early stopping
    ...
    Args:
        ...
        loss_threshold: Loss threshold for early stopping (default: 0.001)
...
def run_classification_phase(kg_path: str, model_path: str, mappings_path: str,
                             ...
                             n_iter: int = 20,
                             cv: int = 3,
                             xgb_loss_threshold: float = 0.001) -> Tuple[XGBClassifier, Dict[str, Any]]:
    """
    Execute Phase 2: Topic Classification with XGBoost early stopping
    ...
    Args:
        ...
        xgb_loss_threshold: XGBoost loss threshold for monitoring (default: 0.001)
    ...
    classifier = TopicClassifier(
        ...
        mappings_path=mappings_path,
        output_dir=output_dir,
        xgb_loss_threshold=xgb_loss_threshold
    )
...
def run_complete_pipeline(kg_path: str, emb_dir: str,
                          ...
                          n_iter: int = 20,
                          cv: int = 3,
                          kge_loss_threshold: float = 0.001,
                          xgb_loss_threshold: float = 0.001) -> Tuple[XGBClassifier, Dict[str, Any]]:
    """
    Execute both phases sequentially with loss-based early stopping
    ...
    Args:
        ...
        n_iter: Number of iterations for RandomizedSearchCV
        cv: Number of cross-validation folds
        kge_loss_threshold: KGE loss threshold for early stopping (default: 0.001)
        xgb_loss_threshold: XGBoost loss threshold for monitoring (default: 0.001)

    Returns:
        Tuple of (best_model, metrics)
    """
    print("STARTING COMPLETE TWO-PHASE PIPELINE WITH LOSS-BASED EARLY STOPPING")
    print("=" * 60)
    print(f"KGE Loss Threshold: {kge_loss_threshold}")
    print(f"XGBoost Loss Threshold: {xgb_loss_threshold}")
    print("=" * 60)
    ...
    model_path = run_kge_training_phase(
        ...
        model_type=model_type,
        embedding_dims=embedding_dims,
        loss_threshold=kge_loss_threshold
    )
...
def example_phase_1():
    """Example of running Phase 1 only with loss threshold"""
    model_path = run_kge_training_phase(
        ...
        loss_threshold=0.001  # NEW: Loss threshold parameter
    )
...
def example_phase_2(model_path: str, mappings_path: str):
    """Example of running Phase 2 only with loss threshold"""
    best_model, metrics = run_classification_phase(
        kg_path="../data/triples_raw/amazon/dataset_triplet_amazon_new_simplificado.csv",
        model_path=model_path,
        mappings_path=mappings_path,
        ...
        n_iter=30,
        cv=5,
        xgb_loss_threshold=0.001  # NEW: XGBoost loss threshold
    )
...
def example_complete_pipeline():
    """Example of running both phases sequentially with loss thresholds"""
    best_model, metrics = run_complete_pipeline(
        ...
        n_iter=25,
        cv=3,
        kge_loss_threshold=0.001,  # NEW: KGE loss threshold
        xgb_loss_threshold=0.001  # NEW: XGBoost loss threshold
    )
...
def interactive_menu():
    """Interactive menu for selecting operations with loss threshold options"""
    print("\n" + "=" * 60)
    ...
    emb_dir = f"{base_path}/triples_emb/{repo}_early_stop"
    model_path = f"{emb_dir}/transe_model.pkl"
    mappings_path = f"{emb_dir}/transe_mappings.pkl"
    output_dir = f"{base_path}/model_output/{repo}_early_stop"

    if choice == '1':
        # Phase 1: KGE Training
        print("\n--- Phase 1: KGE Training ---")
        model_type = input(
            "Enter model type (transe/convkb/complex/distmult/rotate) or press Enter for interactive selection: "
        ).strip().lower() or None

        embedding_dims_input = input(
            "Enter embedding dimensions to test (comma-separated, e.g., 128,192,256) or press Enter for default [128,192,256]: "
        ).strip()
        ...
        # NEW: Ask for loss threshold
        loss_threshold_input = input(
            "Enter KGE loss threshold for early stopping (default: 0.001): "
        ).strip()
        loss_threshold = float(loss_threshold_input) if loss_threshold_input else 0.001
        ...
        # NEW: Ask for XGBoost loss threshold
        xgb_loss_threshold_input = input(
            "Enter XGBoost loss threshold for monitoring (default: 0.001): "
        ).strip()
        xgb_loss_threshold = float(xgb_loss_threshold_input) if xgb_loss_threshold_input else 0.001
        ...
        # NEW: Ask for loss thresholds
        kge_loss_threshold_input = input(
            "Enter KGE loss threshold for early stopping (default: 0.001): "
        ).strip()
        kge_loss_threshold = float(kge_loss_threshold_input) if kge_loss_threshold_input else 0.001
        ...
        best_model, metrics = run_complete_pipeline(
            ...
            n_iter=n_iter,
            cv=cv,
            kge_loss_threshold=kge_loss_threshold,
            xgb_loss_threshold=xgb_loss_threshold
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if len(sys.argv) >= 3:
            ...
            # Validate repository
            if repo not in ['amazon', 'bbc', 'arxiv']:
                print(f"Error: Invalid repository '{repo}'")
                print("Available repositories: amazon, bbc, arxiv")
                sys.exit(1)
            ...
            print(f"Loss threshold: 0.001 (default)")
            print("=" * 60)
            ...
            print("Repositories: amazon, bbc, arxiv")
    else:
        # Interactive mode
        interactive_menu()
