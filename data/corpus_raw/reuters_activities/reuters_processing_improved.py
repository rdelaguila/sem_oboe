...
class ConfigSolution1:
    """Configuration for 4-category approach - Natural language categories"""
    
    TARGET_CATEGORIES = {
        'Corporate_Earnings': 'Corporate_Earnings',
        'International_Trade': 'International_Trade',
        'Energy_Resources': 'Energy_Resources',
        'Agricultural_Markets': 'Agricultural_Markets'
    }
    
    DOCS_PER_CATEGORY = 600  # 600 docs × 4 categories = 2,400 total
    TOTAL_DOCS = 2400
    
    OUTPUT_DIR = './reuters_processed'
    CORPUS_FILE = 'reuters_activities_corpus.csv'
    STATS_FILE = 'reuters_activities_statistics.json'
    RANDOM_SEED = 42
...
class ConfigSolution2:
    ...
    DOCS_PER_CATEGORY = 750
    TOTAL_DOCS = 3000
    
    OUTPUT_DIR = './reuters_activities_processed'
    CORPUS_FILE = 'reuters_activities_corpus.csv'
...
def solution1_nltk_4categories(config):
    """
    Use NLTK Reuters with 4 natural language categories (NO abbreviations).
    
    ADVANTAGES:
    - Balanced dataset (600 docs each)
    - Full text available with natural language
    - NO problematic abbreviations (dlr, yen, dmk, etc.)
    - Excellent for NER and concept map extraction
    
    CATEGORIES:
    - Corporate_Earnings: Company earnings reports, acquisitions, employment news
    - International_Trade: Trade statistics, economic indicators, policy
    - Energy_Resources: Oil, natural gas, strategic resources
    - Agricultural_Markets: Grains, food commodities, agricultural products
    """
    from nltk.corpus import reuters
    import nltk
    
    print("=" * 70)
    print("SOLUTION 1: NLTK Reuters with 4 Natural Language Categories")
    print("=" * 70)
    print("(No abbreviations - optimized for NER and concept maps)")
    print("=" * 70)
    ...
    # Enhanced category mapping - NATURAL LANGUAGE CATEGORIES (No abbreviations)
    category_mapping = {
        # Corporate_Earnings - Company earnings, acquisitions, employment
        'earn': 'Corporate_Earnings',
        'acq': 'Corporate_Earnings',
        'jobs': 'Corporate_Earnings',
        
        # International_Trade - Trade, economic indicators, policy
        'trade': 'International_Trade',
        'reserves': 'International_Trade',
        'bop': 'International_Trade',
        'cpi': 'International_Trade',
        'gnp': 'International_Trade',
        'gdp': 'International_Trade',
        'wpi': 'International_Trade',
        'ipi': 'International_Trade',
        'retail': 'International_Trade',
        'housing': 'International_Trade',
        'income': 'International_Trade',
        'lei': 'International_Trade',
        
        # Energy_Resources - Oil, gas, strategic resources
        'crude': 'Energy_Resources',
        'nat-gas': 'Energy_Resources',
        'strategic-metal': 'Energy_Resources',
        
        # Agricultural_Markets - Grains, food commodities
        'wheat': 'Agricultural_Markets',
        'corn': 'Agricultural_Markets',
        'grain': 'Agricultural_Markets',
        'soybean': 'Agricultural_Markets',
        'coffee': 'Agricultural_Markets',
        'cocoa': 'Agricultural_Markets',
        'sugar': 'Agricultural_Markets',
        'cotton': 'Agricultural_Markets',
        'livestock': 'Agricultural_Markets',
        'meal-feed': 'Agricultural_Markets',
        'oilseed': 'Agricultural_Markets',
        'veg-oil': 'Agricultural_Markets',
        'soy-oil': 'Agricultural_Markets',
        'soy-meal': 'Agricultural_Markets',
        'palm-oil': 'Agricultural_Markets',
        'rapeseed': 'Agricultural_Markets',
        'sunseed': 'Agricultural_Markets',
        'lin-oil': 'Agricultural_Markets',
        'castorseed': 'Agricultural_Markets',
        'groundnut': 'Agricultural_Markets',
        'coconut': 'Agricultural_Markets',
        'copra': 'Agricultural_Markets',
        'rubber': 'Agricultural_Markets',
    }
...
def save_corpus_and_stats(df, config, solution_name):
    ...
    # Save corpus with semicolon separator and quoted text
    corpus_path = os.path.join(config.OUTPUT_DIR, config.CORPUS_FILE)
    df.to_csv(corpus_path, index=False, sep=';', quoting=1)  # quoting=1 means QUOTE_ALL for text fields
    print(f"✓ Corpus saved to: {corpus_path}")
    print(f"  Format: CSV with ';' separator and quoted text fields")
    ...
    train_df.to_csv(train_path, index=False, sep=';', quoting=1)
    test_df.to_csv(test_path, index=False, sep=';', quoting=1)
...
def main():
    ...
    print("\nAvailable solutions:")
    print("1. NLTK Reuters with 4 natural language categories (RECOMMENDED)")
    print("   - 600 docs × 4 categories = 2,400 total")
    print("   - Full text, quick processing, NO abbreviations")
    print("   - Categories: Corporate_Earnings, International_Trade,")
    print("                 Energy_Resources, Agricultural_Markets")
    print()
    print("2. sklearn RCV1 with 4 categories (if NLTK doesn't work)")
    print("   - 600 docs × 4 categories = 2,400 total")
    print("   - Requires text reconstruction")
    print()
    print("3. Merged categories (alternative - 3 categories)")
    ...
    df = solution1_nltk_4categories(config)
    stats = save_corpus_and_stats(df, config, "Solution 1: 4 Categories")
    ...
    print("\n📁 Files created:")
    print(f"  1. {config.OUTPUT_DIR}/reuters_corpus.csv")
    ...
    print("""
    "We selected four thematically coherent categories from Reuters corpus:
    Corporate_Earnings, International_Trade, Energy_Resources, and 
    Agricultural_Markets. Each category contains 600 documents, totaling 
    2,400 documents.
    
    These categories were chosen to maximize natural language content and
    facilitate entity extraction. Unlike currency-focused categories with
    dense technical abbreviations, these provide rich semantic content with
    clear named entities (companies, countries, products, organizations) and
    meaningful predicates, essential for high-quality concept map generation.
    
    The corpus spans diverse economic sectors (corporate finance, international
    trade, energy, agriculture), validating OBOE's cross-domain generalization
    alongside Amazon (informal reviews) and BBC (regional news). This
    demonstrates robust performance across textual styles and subject domains."
    """)
    
    print("\n" + "=" * 70)
    print("Next steps:")
    print("1. Verify: reuters_processed/reuters_corpus.csv")
    print("2. Run OBOE pipeline on this corpus")
    print("3. Update tables with '4 categories' as planned")
